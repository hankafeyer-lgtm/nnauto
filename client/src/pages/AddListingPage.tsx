...
import { Sparkles, Car, Package, Wrench, CircleDot, Zap, Bot, Activity, ArrowUp, ArrowDown, Grid3x3, Compass, Key, MapPin, Loader2, CheckCircle2, AlertCircle, Info } from "lucide-react";
import { buildAutofillPatch, type AutofillData } from "@/lib/vinAutofillMap";
...
export default function AddListingPage() {
  ...
  // VIN auto-fill (Přidat inzerát) – isolated state, never required for submit.
  // `partial` covers two cases: (a) NHTSA returned no/limited data, and
  // (b) backend responded with a 4xx that's not a network/server fault.
  // `error` is reserved for true technical failures (5xx, 429, network).
  const [autofillVin, setAutofillVin] = useState("");
  const [autofillLoading, setAutofillLoading] = useState(false);
  const [autofillStatus, setAutofillStatus] = useState<
    | { type: "success" | "partial" | "error"; message: string }
    | null
  >(null);
  // Friendly labels of fields that were just auto-filled, shown under the
  // status alert so the user knows which inputs were touched.
  const [autofillFilledLabels, setAutofillFilledLabels] = useState<string[]>(
    [],
  );
  // Track which VIN we have already decoded so we never re-spam NHTSA.
  // We use both a ref (read synchronously inside async handler – avoids stale
  // closures) and state (used for re-rendering the disabled-button state).
  const lastDecodedVinRef = useRef<string | null>(null);
  const [lastDecodedVin, setLastDecodedVin] = useState<string | null>(null);
  // Snapshot of the values we wrote during the previous successful autofill.
  // If the form value differs from this snapshot, the user manually edited
  // the field and we MUST NOT overwrite their change on a subsequent run.
  const autofilledValuesRef = useRef<Record<string, unknown>>({});
  // Debounce handle for the auto-trigger (no-button) flow.
  const autofillDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  // VIN auto-fill — POSTs to /api/vin/decode and patches matching form
  // fields via setValue. Behavior:
  //   • Auto-triggered after a 600ms debounce when the user has typed a
  //     valid 17-character VIN. The "Vyplnit automaticky" button is kept
  //     as an explicit fallback.
  //   • Never overwrites a field the user has manually edited after a
  //     previous autofill (compares against `autofilledValuesRef`).
  //   • Never decodes the same VIN twice in a row.
  //   • Status:
  //       ≥ 2 of {brand, model, year} filled → "success" (green)
  //       success:false / thin data         → "partial" (amber)
  //       4xx that isn't network/server     → "partial"
  //       5xx / 429 / network failure       → "error" (red)
  const performAutofill = useCallback(
    async (rawVin: string): Promise<void> => {
      const vin = (rawVin || "")
        .toUpperCase()
        .replace(/\s+/g, "")
        .replace(/[^A-Z0-9]/g, "");
      if (vin.length !== 17 || /[IOQ]/.test(vin)) return;
      // No-op if we already decoded this exact VIN. The button is disabled
      // in this state, but the auto-trigger effect can race – guard here too.
      if (lastDecodedVinRef.current === vin) return;

      setAutofillLoading(true);
      setAutofillStatus(null);

      // Helper: write a value to the form ONLY if it is safe to do so.
      // "Safe" means the field is currently empty, or its value matches the
      // value WE wrote last time (i.e. the user has not touched it since).
      ...
      let keyFieldsFilled = 0;
      const tryFill = <K extends keyof InsertListing>(
        name: K,
        newValue: unknown,
        label: string | null,
        isKey: boolean,
        ...
        const prev = autofilledValuesRef.current[name as string];
        if (!isEmpty(current) && !equal(current, prev)) return; // user-owned
        form.setValue(name, newValue as any, { shouldDirty: true });
        autofilledValuesRef.current[name as string] = newValue;
        if (label) filledLabels.push(label);
        if (isKey) keyFieldsFilled++;
      };

      try {
        const res = await apiRequest("POST", "/api/vin/decode", { vin });
        const json = (await res.json()) as {
          success: boolean;
          data: AutofillData | null;
        };

        // Always sync the canonical VIN field so the existing VIN input
        // lower in the form mirrors the autofill input value.
        form.setValue("vin", vin, { shouldDirty: true });

        if (json.success && json.data) {
          const patch = buildAutofillPatch(json.data, {
            vehicleType: form.getValues("vehicleType") ?? null,
            brand: form.getValues("brand") ?? null,
          });

          if (patch.vehicleType) {
            tryFill("vehicleType", patch.vehicleType, null, false);
          }
          if (patch.brand) tryFill("brand", patch.brand, "značka", true);
          if (patch.model) tryFill("model", patch.model, "model", true);
          if (patch.year != null) tryFill("year", patch.year, "rok", true);
          if (patch.bodyType)
            tryFill("bodyType", patch.bodyType, "karoserie", false);
          if (patch.fuelType)
            tryFill("fuelType", patch.fuelType, "palivo", false);
          if (patch.transmission)
            tryFill("transmission", patch.transmission, "převodovka", false);
          if (patch.engineVolume)
            tryFill("engineVolume", patch.engineVolume, "objem motoru", false);
        }

        // Lock further auto-triggers / button clicks for THIS exact VIN.
        lastDecodedVinRef.current = vin;
        setLastDecodedVin(vin);
        setAutofillFilledLabels(filledLabels);

        if (keyFieldsFilled >= 2) {
          setAutofillStatus({
            type: "success",
            message:
              "Údaje byly automaticky doplněny. Zkontrolujte je prosím.",
          });
        } else {
          setAutofillStatus({
            type: "partial",
            message:
              "VIN se nepodařilo plně rozpoznat. Některé údaje vyplňte ručně.",
          });
        }
      } catch (err) {
        ...
        // Don't lock the VIN on technical failures – the user should be able
        // to retry without changing the VIN.
        if (!isTechnicalFailure) {
          lastDecodedVinRef.current = vin;
          setLastDecodedVin(vin);
        }
        setAutofillFilledLabels([]);
        setAutofillStatus({
          type: isTechnicalFailure ? "error" : "partial",
          message: isTechnicalFailure
            ? "Nepodařilo se načíst údaje podle VIN. Zkuste to prosím znovu."
            : "VIN se nepodařilo plně rozpoznat. Některé údaje vyplňte ručně.",
        });
      } finally {
  ...
  // Auto-trigger: when the user has typed a complete, valid, NEW VIN, fire
  // the autofill after a short debounce. Most users never need to click the
  // button. The cleanup keeps things sane while the user is still typing.
  useEffect(() => {
    if (autofillDebounceRef.current) {
      clearTimeout(autofillDebounceRef.current);
      autofillDebounceRef.current = null;
    }
    if (autofillVin.length !== 17) return;
    if (/[IOQ]/.test(autofillVin)) return;
    if (lastDecodedVinRef.current === autofillVin) return;
    if (autofillLoading) return;

    autofillDebounceRef.current = setTimeout(() => {
      void performAutofill(autofillVin);
    }, 600);
  ...
  }, [autofillVin, autofillLoading, performAutofill]);
          ...
          <Card className="rounded-xl sm:rounded-2xl shadow-xl">
            ...
            <CardContent>
              <Form {...form}>
                ...
                >
                  {/*
                    VIN auto-fill block (optional). Sends the VIN to
                    /api/vin/decode (NHTSA-backed) and pre-fills brand /
                    model / year / body / fuel / engine / transmission. The
                    user can still adjust everything manually afterwards. If
                    the VIN cannot be decoded, the form keeps working as
                    before – this block never blocks submit.
                  */}
                  {(() => {
                    // Local helpers – computed at render time, no state.
                    // Validation messages are intentionally subtle (small
                    // text under the input) and ONLY surface when there's
                    // an actual problem with the current input. Otherwise
                    // a neutral hint is shown.
                    const vinHasInvalidChars = /[IOQ]/.test(autofillVin);
                    const vinIsCorrectLength = autofillVin.length === 17;
                    const vinIsValid = vinIsCorrectLength && !vinHasInvalidChars;
                    const validationMsg = vinHasInvalidChars
                      ? "VIN kód obsahuje neplatné znaky."
                      : autofillVin.length > 0 && !vinIsCorrectLength
                        ? "VIN kód musí mít 17 znaků."
                        : null;
                    // Avoid spamming NHTSA: once we have a result for THIS
                    // exact VIN, the button is disabled until the user edits
                    // the VIN (or clears it).
                    const alreadyDecodedThisVin =
                      vinIsCorrectLength && lastDecodedVin === autofillVin;
                    return (
                      <div
                        className="rounded-xl border border-border/60 bg-muted/30 p-4 sm:p-5 space-y-3"
                        data-testid="vin-autofill-block"
                      >
                        <div className="space-y-1">
                          <label
                            htmlFor="vin-autofill-input"
                          ...
                          <p className="text-xs text-muted-foreground">
                            Zadejte VIN a vyplňte základní údaje automaticky.
                            Pole VIN je nepovinné.
                          </p>
                        </div>
                        <div className="flex flex-col sm:flex-row gap-2">
                          <Input
                            id="vin-autofill-input"
                            placeholder="Zadejte VIN kód"
                            value={autofillVin}
                            maxLength={17}
                            onChange={(e) => {
                              // Trim + uppercase + drop spaces and stray
                              // punctuation. Cursor stays at the end because
                              // each typed char maps 1:1 to an output char.
                              const next = e.target.value
                                .toUpperCase()
                                .replace(/\s+/g, "")
                                .replace(/[^A-Z0-9]/g, "")
                                .slice(0, 17);
                              if (next === autofillVin) return;
                              setAutofillVin(next);
                              // Clear any previous result UI – it belonged to
                              // the OLD VIN and would otherwise stay visible
                              // while the user types a new one.
                              if (autofillStatus) setAutofillStatus(null);
                              if (autofillFilledLabels.length > 0) {
                                setAutofillFilledLabels([]);
                              }
                            }}
                            ...
                            aria-describedby="vin-autofill-helper"
                            data-testid="input-vin-autofill"
                          />
                          <Button
                            type="button"
                            variant="default"
                            ...
                            disabled={
                              autofillLoading ||
                              !vinIsValid ||
                              alreadyDecodedThisVin
                            }
                            className="sm:w-auto w-full"
                            data-testid="button-vin-autofill"
                          >
                            {autofillLoading ? (
                              <>
                        ...
                        {/* Tiny conversion booster – nudges the user that
                            the VIN flow saves time. Stays subtle so it
                            doesn't compete with the validation/hint text. */}
                        <p
                          className="text-[11px] text-muted-foreground/80"
                          data-testid="vin-autofill-booster"
                        >
                          Ušetří vám až 2 minuty vyplňování.
                        </p>

                        {/* Hint or validation – exactly one of them is
                            visible at a time, so the block height stays
                            stable while the user types. */}
                        <p
                          id="vin-autofill-helper"
                          className={
                            validationMsg
                          ...
                          data-testid={
                            validationMsg
                              ? "vin-autofill-validation"
                              : "vin-autofill-hint"
                          }
                        >
                          {validationMsg ??
                            "VIN najdete v technickém průkazu vozidla."}
                        </p>
                        ...
                        {autofillStatus ? (
                          <div
                            ...
                            aria-live="polite"
                            className={
                              "transition-opacity duration-300 motion-safe:animate-in motion-safe:fade-in-0 motion-safe:slide-in-from-top-1 " +
                              (autofillStatus.type === "success"
                                ? "flex items-start gap-2 rounded-md border border-green-600/30 bg-green-50 dark:bg-green-950/30 px-3 py-2 text-xs text-green-800 dark:text-green-300"
                                : autofillStatus.type === "error"
                                  ...
                                  : "flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-50 dark:bg-amber-950/30 px-3 py-2 text-xs text-amber-800 dark:text-amber-300")
                            }
                            data-testid="vin-autofill-status"
                            data-status={autofillStatus.type}
                          >
                            {autofillStatus.type === "success" ? (
                            ...
                            <div className="space-y-1">
                              ...
                              {autofillFilledLabels.length > 0 ? (
                                <p
                                  className="text-[11px] opacity-80"
                                  data-testid="vin-autofill-filled"
                                >
                                  Vyplněno: {autofillFilledLabels.join(", ")}
                                </p>
                              ) : null}
                            </div>
                          </div>
