...
/**
 * Idempotently ensure the messaging tables (conversations, messages,
 * quick_replies) exist. Runs once per process — safe to invoke from any
 * messaging API route.
 *
 * The deploy pipeline (.github/workflows/deploy-vps.yml) does not run
 * drizzle-kit migrate, so this guarantees the inbox storage is ready
 * the first time it is exercised in production. Mirrors the pattern of
 * lib/ensurePasswordResetSchema.ts.
 */
  ...
  if (!ensured) {
    ensured = (async () => {
      try {
        ...
        ensured = null;
        console.error("[ensureMessagingSchema] Failed to ensure schema:", e);
        throw e;
      }
    })();
  }
