...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!listingId) return error("Listing with VIN is required", 400);
    ...
    if (!/^[A-HJ-NPR-Z0-9]{17}$/.test(listingVin)) {
      return error("VIN is not available for this listing", 400);
    }

    const vinFromBody = typeof body?.vin === "string" ? body.vin.trim().toUpperCase() : "";
    ...
    if (CEBIA_STRIPE_PAYMENT_LINK_URL) {
      ...
      return json({ url: u.toString(), reportId: report.id, guestToken });
    }
    ...
    console.error("Cebia guest checkout error:", e);
